<!DOCTYPE html> <!-- The new doctype -->
<html>
    <head>
        <meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
        <title>Cricket Cricket</title>
        <link rel="stylesheet" type="text/css" media="screen" href="css/style.css" />
		<link rel="stylesheet" type="text/css" media="print" href="css/print-style.css" />
		<link rel="shortcut icon" href="favicon.ico" />
        
       
    </head>
    <body>
    	
               
    	<section id="page"> <!-- Defining the #page section with the section tag -->
    <p id="skipnav"><a href="#container">Skip navigation</a></p>
            <header> <!-- Defining the header section of the page with the appropriate tag -->

                <hgroup>
				
				
<div class="image-header"> <img src="img/logo-2.jpg"   alt="Our Logo" /> 
  <p class="title">Cricket Cricket</p>
  <img src="img/title.jpg"   alt="title" class="right-align" /> 
  <p class="slogan">World's most popular game</p>
</div>
                  
                </hgroup>
                                
               <nav class="clear" > <!-- The nav link semantically marks your main site navigation -->
                    <ul>
                        <li><a href="index.php">Home</a></li>
                        <li><a href="about-cricket.php">About Cricket</a></li>
                        <li><a href="history.php">Cricket History</a></li>
						 <li><a href="equipements.php">Cricket Equipments</a></li>
                        <li><a href="cricket-rules.php">Cricket Rules</a></li>
                        <li><a href="famous-teams.php">Famous Cricket Teams</a></li>
						<!--<li><a href="join.php">Join Us</a></li>!-->
                    </ul>
                </nav>
            
            </header>
			    <div class="line"></div>
			<section class="container"> 
<!-- A new section with the articles -->

 <div class="page-nav">
 	<li><a href="#ODI">ODI Ranking</a></li>
  	<li><a href="#Test">Test Ranking</a></li>
  	<li><a href="#T20">T20 Ranking</a></li>
 
 </div>
<!-- Article 1 start -->
<article id="article1"> 
  <!-- The new article tag. The id is supplied so it can be scrolled into view. -->
  <h2>Famous Cricket Teams with their Ranking</h2>
  
 
  <article class="left"  id="ODI">
	<h2>ODI Ranking</h2> 
    <table class="ranking" >
      <tbody>
        <tr> 
          <th>Rank</th>
          <th>Team</th>
          <th>Matches</th>
          <th>Points</th>
          <th>Rating</th>
        </tr>
        <tr> 
          
      <td>1</td>
          
      <td> <span class="tLogoEngland"></span> England</td>
          
      <td>56</td>
          
      <td>6967</td>
          
      <td>124</td>
        </tr>
        <tr> 
          
      <td>2</td>
          
      <td><span class="tLogoIndia"></span> India</td>
          
      <td>67</td>
          
      <td>7939</td>
          
      <td>118</td>
        </tr>
        <tr> 
          
      <td>3</td>
          
      <td><span class="tLogoNewZealand"></span> New Zealand</td>
          
      <td>46</td>
          
      <td>5347</td>
          
      <td>116</td>
        </tr>
        <tr> 
          
      <td>4</td>
          
      <td><span class="tLogoSouthAfrica"></span> South Africa</td>
          
      <td>49</td>
          
      <td>5442</td>
          
      <td>111</td>
        </tr>
        <tr> 
          
      <td>5</td>
          
      <td><span class="tLogoAUS"></span> Australia</td>
          
      <td>53</td>
          
      <td>5854</td>
          
      <td>110</td>
        </tr>
        <tr> 
          
      <td>6</td>
          
      <td><span class="tLogoPakistan"></span> Pakistan</td>
          
      <td>51</td>
          
      <td>5019</td>
          
      <td>98</td>
        </tr>
        <tr> 
          
      <td> 7 </td>
          
      <td><span class="tLogoBanglaDesh"></span> Bangladesh</td>
          
      <td>46</td>
          
      <td>3963</td>
          
      <td>86</td>
        </tr>
        <tr> 
          
      <td> 8 </td>
          
      <td><span class="tLogoSriLanka"></span> Sri Lanka</td>
          
      <td>56</td>
          
      <td>4520</td>
          
      <td>81</td>
        </tr>
        <tr> 
          
      <td> 9 </td>
          
      <td><span class="tLogoWestIndies"></span> West Indies</td>
          
      <td>57</td>
          
      <td>4573</td>
          
      <td>80</td>
        </tr>
        <tr> 
          
      <td> 10 </td>
          
      <td><span class="tLogoAfghanistan"></span> Afghanistan</td>
          
      <td>43</td>
          
      <td>2440</td>
          
      <td>57</td>
        </tr>
        <tr> 
          
      <td> 11 </td>
          
      <td><span class="tLogoIreland"></span> Ireland</td>
          
      <td>31</td>
          
      <td>1525</td>
          
      <td>49</td>
        </tr>
        <tr> 
          
      <td> 12 </td>
          
      <td><span class="tLogoZimbabwe"></span> Zimbabwe</td>
          
      <td>35</td>
          
      <td>1538</td>
          
      <td>44</td>
        </tr>
		
		
      </tbody>
    </table>
	<a href="#" class="up">Top</a>
	</article>
	
	  <div  class="clear">
	
    <article class="right"  id="Test"> 
	<h2>Test Ranking</h2>
      <table class="ranking">
        <tbody>
          <tr> 
            <th>Rank</th>
            <th>Team</th>
            <th>Matches</th>
            <th>Points</th>
            <th>Rating</th>
          </tr>
          <tr> 
            
        <td>1</td>
            
        <td> <span class="tLogoAUS"></span> Australia</td>
            
        <td>32</td>
            
        <td>3950</td>
            
        <td>123</td>
          </tr>
          <tr> 
            
        <td>2</td>
            
        <td><span class="tLogoSouthAfrica"></span> South Africa</td>
            
        <td>23</td>
            
        <td>2831</td>
            
        <td>123</td>
           </tr>
          <tr> 
            
        <td>3</td>
            
        <td><span class="tLogoEngland"></span> England</td>
            
        <td>30</td>
            
        <td>3131</td>
            
        <td>104</td>
		  </tr>
          <tr> 
            
        <td>4</td>
            
        <td><span class="tLogoPakistan"></span> Pakistan</td>
            
        <td>20</td>
            
        <td>2064</td>
            
        <td>103</td>
		   </tr>
          <tr> 
            
        <td>5</td>
            
        <td><span class="tLogoIndia"></span> India</td>
            
        <td>23</td>
            
        <td>2343</td>
            
        <td>102</td>
		   </tr>
          <tr> 
            
        <td>6</td>
            
        <td><span class="tLogoNewZealand"></span> New Zealand</td>
            
        <td>26</td>
            
        <td>2383</td>
            
        <td>92</td>
          </tr>
          <tr> 
            
        <td> 7 </td>
		    
        <td><span class="tLogoSriLanka"></span> Sri Lanka</td>
            
        <td>22</td>
            
        <td>1986</td>
            
        <td>90</td>
		  </tr>
          <tr> 
            
        <td> 8 </td>
            
        <td><span class="tLogoWestIndies"></span> West Indies</td>
            
        <td>19</td>
            
        <td>1442</td>
            
        <td>76</td>
          </tr>
          <tr> 
            
        <td> 9 </td>
            
        <td><span class="tLogoZimbabwe"></span> Zimbabwe</td>
            
        <td>8</td>
            
        <td>322</td>
            
        <td>40</td>
          </tr>
          <tr> 
            
        <td> 10 </td>
            
        <td><span class="tLogoBanglaDesh"></span> Bangladesh</td>
            
        <td>14</td>
            
        <td>287</td>
            
        <td>21</td>		
          </tr>
          <tr> 
            
        <td> 11 </td>
            
        <td><span class="tLogoIreland"></span> Ireland</td>
            
        <td>8</td>
            
        <td>273</td>
            
        <td>34</td>
          </tr>
          <tr> 
            
        <td> 12 </td>
            
        <td><span class="tLogoAfghanistan"></span> Afghanistan</td>
            
        <td>9</td>
            
        <td>304</td>
            
        <td>34</td>
          </tr>
		 	
        </tbody>
      </table>
	   <a href="#" class="up">Top</a>
    </article>
	
 
  <article class="left"  id="T20"> 
	<h2>T20 Ranking</h2>
      <table class="ranking">
        <tbody>
          <tr> 
            <th>Rank</th>
            <th>Team</th>
            <th>Matches</th>
            <th>Points</th>
            <th>Rating</th>
          </tr>
          <tr> 
            
        <td>1</td>
            
        <td><span class="tLogoIndia"></span> India</td>
            
        <td>15</td>
            
        <td>1963</td>
            
        <td>131</td>
			
          </tr>
          <tr> 
            
        <td>2</td>
            
        <td><span class="tLogoSriLanka"></span> Sri Lanka</td>
            
        <td>22</td>
            
        <td>2859</td>
            
        <td>130</td>
			
           </tr>
          <tr> 
            
        <td>3</td>
            
        <td><span class="tLogoPakistan"></span> </td>
            
        <td>26</td>
            
        <td>3194</td>
            
        <td>123</td>
			
		  </tr>
          <tr> 
            
        <td>4</td>
            
        <td><span class="tLogoSouthAfrica"></span> South Africa</td>
            
        <td>23</td>
            
        <td>2785</td>
            
        <td>121</td>
		   </tr>
          <tr> 
            
        <td>5</td>
            
        <td> <span class="tLogoAUS"></span> Australia</td>
            
        <td>22</td>
            
        <td>2454</td>
            
        <td>112</td>
		   </tr>
          <tr> 
            
        <td>6</td>
            
        <td><span class="tLogoNewZealand"></span> New Zealand</td>
            
        <td>20</td>
            
        <td>2196</td>
            
        <td>110</td>
          </tr>
          <tr> 
            
        <td> 7 </td>
		    
        <td><span class="tLogoWestIndies"></span> West Indies</td>
            
        <td>23</td>
            
        <td>2520</td>
            
        <td>110</td>
		  </tr>
          <tr> 
            
        <td> 8 </td>
            
        <td><span class="tLogoEngland"></span> England</td>
            
        <td>23</td>
            
        <td>2220</td>
            
        <td>97</td>
			
          </tr>
          <tr> 
            
        <td> 9 </td>
            
        <td><span class="tLogoIreland"></span> Ireland</td>
            
        <td>12</td>
            
        <td>1046</td>
            
        <td>87</td>
		    
          </tr>
          <tr> 
            
        <td> 10 </td>
            
        <td><span class="tLogoBanglaDesh"></span> Bangladesh</td>
            
        <td>16</td>
            
        <td>1147</td>
            
        <td>72</td>		
          </tr>
          <tr> 
            
        <td> 11 </td>
            
        <td><span class=""></span> Natherlands</td>
            
        <td>14</td>
            
        <td>951</td>
            
        <td>68</td>
          </tr>
          <tr> 
            
        <td> 12 </td>
            
        <td><span class="tLogoAfghanistan"></span> Afghanistan</td>
            
        <td>12</td>
            
        <td>743</td>
            
        <td>62</td>
          </tr>
		  <tr> 
            
        <td> 13 </td>
		  
        <td><span class="tLogoZimbabwe"></span> Zimbabwe</td>
            
        <td>11</td>
            
        <td>573</td>
            
        <td>52</td>
			</tr>
			<tr> 
            
        <td> 14 </td>
		  
        <td><span class=""></span> Scotland</td>
            
        <td>10</td>
            
        <td>512</td>
            
        <td>51</td>
			</tr>
        </tbody>
      </table>
	    <a href="#" class="up">Top</a>
    </article>
	</div>
</article>
<!--<h4> Source: <a href="http://www.icc-cricket.com/team-rankings/odi">ICC-Cricket</a></h4>--> 
<!-- Article 1 end -->
</section> 
<footer> 
<!-- Marking the footer section -->
<div class="line"></div>
<p>Copyright &copy 2014 - Cricket Cricket</p>
<!-- copyright notice -->
<a href="#" class="up">Go on Top</a> </footer> </section> 
<!-- Closing the #page section -->
</body>
</html>
